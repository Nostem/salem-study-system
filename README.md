...
| Static quiz bank | 898 imported written exam questions (563 quiz-eligible) |
...
This prevents a browser from reusing stale/corrupted Pagefind binary assets after a Vercel deploy. The Pagefind assets are served with `no-transform` (they are pre-gzipped and decompressed in JS) and a revalidatable `Cache-Control` (`max-age`/`stale-while-revalidate`, **not** `immutable`) so a page reload can recover a bad cache entry — `immutable` would tell the browser to reuse the cached copy even on refresh. If Pagefind reports a stale WASM pointer, the search modal retries once with a cache-busted module. Use `npm run build:astro` when a check only needs rendered pages and does not need Pagefind search indexing.

**Fallback search.** Some proxies/security appliances ignore `no-transform` and corrupt or block Pagefind's binary WASM/index files, breaking search on those networks even though it works elsewhere (e.g. on a phone). When Pagefind fails to load or search, the modal transparently falls back to a text-only index at `/search-fallback.json` — a plain JSON artifact (title + trimmed body text per public wiki article) emitted by the `src/pages/search-fallback.json.ts` endpoint, which survives proxy transforms that mangle the raw-gzip binaries. Results are marked "Basic search". Only if Pagefind **and** the fallback both fail does the modal show the one-click **Reload page** action. The ranking/text logic lives in `src/utils/search-fallback.ts` (unit-tested); the modal mirrors the ranker inline (it is an inline script) and is integration-tested in `tests/ui-test.ts`.
