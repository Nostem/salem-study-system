...
The Compressed Air System provides the station with a reliable supply of clean, oil-free air which is directed to various locations for services as required (UFSAR §9.3.1). The system provides a reliable supply of clean, oil-free, dry air at temperatures and pressures suitable for use as control air and for containment penetration cooling, as well as for miscellaneous services and maintenance (UFSAR §9.3.1.1). The Compressed Air System is designed such that any single failure will not result in loss of function (UFSAR §9.3.1.1).

The Control Air System for each unit consists of a dual header arrangement supplied through two distinct parallel paths: one path is supplied from the Unit 1 Station Air System and the other is supplied from the Unit 2 Station Air System (UFSAR §9.3.1.2.2). Control air for the safety-related portions is automatically backed up by an emergency control air compressor, and control air is fed from the Station Air System through heatless, desiccant-type air dryers (UFSAR §9.3.1.2.2). The Emergency Control Air System is designated Class I (seismic) and is located in a Class I (seismic) structure (UFSAR §9.3.1.2.2).

The Control Air System is designed to supply the required air during normal and abnormal conditions; any single component failure will not result in a loss of function, and a total loss of control air to all systems and equipment is therefore not considered credible (UFSAR §9.3.1.3). Redundant safety-related air users are provided with independent single air supplies from the control air header system, and the redundant system instruments have control air supplied from independent air headers (UFSAR §9.3.1.3). Separate and redundant headers, backed up by emergency control air compressors energized from the Standby Power System, together with preservation of header independence, assure that air is available during normal as well as abnormal plant conditions, and a single failure within the system would not result in total loss of air supply to redundant equipment (UFSAR §9.3.1.3).

## Key Components

- **Station air compressors:** Three motor-driven, oil-free, centrifugal compressors which draw air from the atmosphere; the intakes are located to avoid drawing in toxic or corrosive gases (UFSAR §9.3.1.2.1). Each compressor has a capacity of <span class="hi">3300 scfm at 120 psig</span> discharge pressure, and the dual station service air headers are fed by three 100-percent capacity air compressors, any one of which can supply the total service and control air requirements for both units (UFSAR §9.3.1.2.1). One compressor is typically running to satisfy the normal requirements of station air and control air for both units as well as to supply containment penetration cooling air for both units; a second compressor serves as standby, and a third compressor is available when required (UFSAR §9.3.1.2.1). Each compressor is furnished with an <span class="hi">800-hp motor</span>, intake filter-silencer, blow-off silencer and digital pressure controls only, intercoolers, aftercooler, and moisture separator (UFSAR §9.3.1.2.1).
- **Service air headers and receivers:** The compressors discharge into two independent service air headers, with an air receiver tied to each header (UFSAR §9.3.1.2.1). The station air header for each unit is supplied from either of the two service air headers and provides operating and service requirements at various locations (UFSAR §9.3.1.2.1).
- **Containment penetration cooling supply:** The containment penetration cooling system for each unit is furnished with two supply lines — the normal supply is taken from the station air header and the backup supply from either of the two service air headers (UFSAR §9.3.1.2.1).
- **Emergency Control Air Compressors (ECACs):** Each system has an emergency control air compressor complete with its own dryer and accessories to supply the safety-related headers; the emergency control air for either system may be directed to supply air for the opposite system through a valved connection (UFSAR §9.3.1.2.1). Each ECAC motor is energized from the standby AC power supply (UFSAR §9.3.1.2.1). Each emergency control air compressor has a capacity of <span class="hi">500 scfm at 110 psig</span> and is driven by a <span class="hi">125-hp motor</span> (UFSAR §9.3.1.2.1). Accessory equipment includes an independent heatless desiccant-type air dryer, intake filter, silencer, intercooler, aftercooler, moisture separator, inlet control valve, relief valve, and automatic condensate trap and drain; the ECACs have Teflon piston rings and stainless steel cylinder liners (UFSAR §9.3.1.2.1). Cooling of the emergency control air compressors is provided from the safety-related Chilled Water System, and in the event the Chilled Water System is not available the Service Water System (SWS) serves as a backup (UFSAR §9.3.1.2.1).
- **Control air headers and receivers:** Each unit's control air system is a dual (parallel) header arrangement (UFSAR §9.3.1.2.2). The parallel control air headers for each unit include air receivers of sufficient capacity to dampen pressure surges and act as a momentary reservoir of air to permit switching of air supply sources without cycling the system (UFSAR §9.3.1.2.1, §9.3.1.3). Reserve air storage of sufficient capacity to minimize pressure fluctuation is provided by means of air receivers in each control air header (UFSAR §9.3.1.3).
- **Air dryers:** Control air is fed from the Station Air System through heatless, desiccant-type air dryers; each ECAC has its own independent heatless desiccant-type dryer (UFSAR §9.3.1.2.2, §9.3.1.2.1).
- **SBO emergency diesel-driven air compressor:** The Compressed Air System is also backed up by the Station Blackout (SBO) emergency diesel-driven air compressor through a connection to Header 2A (UFSAR §9.3.1.2.1). It is located in the Station Blackout Compressor Building and draws in outside air through louvers built into the building wall; the supply air passes through a filter and a regulating inlet valve, is compressed, then passes through an air/oil separator (where oil is removed and reused for compressor lubrication), an aftercooler, a centrifugal separator and moisture trap at the aftercooler outlet, and a coalescing filter to meet hydrocarbon content requirements (UFSAR §9.3.1.2.1). The air is then passed through a heatless desiccant air dryer with an additional outlet filter to remove desiccant fines, and supplied to the emergency control air header via yard access valve <span class="hi">2CA584</span>, which provides air to all required safety-related controls (UFSAR §9.3.1.2.1). The SBO Emergency Compressed Air System is physically isolated and independent of the plant safety-related components and system (UFSAR §9.3.1.3).

## Operation

One station air compressor is typically running to satisfy the normal requirements of station air and control air for both units and to supply containment penetration cooling air, with a second compressor on standby and a third available when required (UFSAR §9.3.1.2.1). Control air is normally supplied from the Station Air System through the heatless desiccant air dryers (UFSAR §9.3.1.2.2).

In addition to the normal air supply from the service air headers, each system has an emergency control air compressor to supply the safety-related headers (UFSAR §9.3.1.2.1). The emergency control air compressors may be operated in three modes: remote manual operation from the Control Room, local manual operation from the "Hot Shutdown" panel, and automatic start-stop operation; normally the emergency air compressors are in the automatic mode (UFSAR §9.3.1.5.2). The ECAC motors are started by either tripping of all three station air compressors or decay of control air header pressure below <span class="val-trip">85 psig</span>, as sensed by a pressure switch in the respective control air header (UFSAR §9.3.1.5.2).

The SBO air compressor is designed to be manually started in the event of a loss of offsite power coincident with the failure of either the "C" emergency diesel generator or the emergency air compressor in the non-blacked-out unit; it may also be used to supply limited control air in the event of a complete loss of control air due to reasons other than an SBO event (UFSAR §9.3.1.2.1, §9.3.1.5.2). The SBO Emergency Compressed Air System is manually started within one hour following an SBO event, and the compressor trips automatically if any of its operating parameters (engine oil pressure, discharge air temperature, and engine water temperature) exceed their design limits (UFSAR §9.3.1.3, §9.3.1.5.2). The station air compressor control circuits are designed to protect the compressor against low oil pressure, high air temperature, low/high oil temperature, low cooling water pressure, and excessive vibration (UFSAR §9.3.1.5.1).

The design evaluation identifies the following failures and resultant actions (UFSAR §9.3.1.3): on loss of a station air compressor, the spare air compressor automatically supplies the total requirements for both units; on loss of all station air compressors, each emergency control air compressor (energized from its standby AC power supply) will supply all control air requirements for its safety-related headers; and on loss of offsite power with an emergency compressor failing to start, the emergency air receivers on each control air header supply enough capacity to maintain header pressure while the second emergency air compressor can provide the safety-related control air requirements for both units (UFSAR §9.3.1.3).

### Air-Operated Valve Fail Modes
...
### SBO Compressor and Loss-of-Air Scenarios
